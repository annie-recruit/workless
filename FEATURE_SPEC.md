...
- 연결선 시각화
- 미니맵을 통한 전체 보기
- **액션 프로젝트**: 선택된 메모리들로부터 실행 가능한 단계별 계획(Action Plan) 생성
- **AI 초안 (AI Draft)**: 각 실행 항목(Action)에 대해 첨부파일 및 외부 링크 소스를 분석하여 구체적인 실행 초안 작성
...
### 6.6 액션 프로젝트 및 AI 초안
- **프로젝트 생성**: 여러 메모리를 선택하여 AI가 단계별(Milestone) 및 실행 항목(Action)으로 구성된 프로젝트 생성
- **소스 기반 분석**: 프로젝트에 연결된 소스 메모리(PDF 첨부파일, 노션 링크 등)를 직접 파싱하여 분석 재료로 활용
- **AI 초안 생성**: 특정 액션에 대해 소스 데이터를 바탕으로 STAR 방식 등의 구체적 실행 초안 작성
- **UI/UX**: 픽셀 아이콘 기반의 로딩 애니메이션 및 다국어(초안/Draft) 지원

#### API 엔드포인트
- `POST /api/projects/[id]/generate-draft` - 소스 데이터를 기반으로 특정 액션의 초안 생성
...
| POST | `/api/projects/[id]/generate-draft` | 액션 프로젝트 초안 생성 |
