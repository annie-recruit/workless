...
### Q: 테스트 환경에서 다른 이메일로 접속이 되었는데 갑자기 안 돼요
A: 이는 Google OAuth 테스트 모드의 제한 사항 때문입니다. 가능한 원인과 해결 방법:

**원인:**
1. **테스트 사용자 승인 만료**: 테스트 사용자의 승인은 **7일 후 만료**됩니다. 만료 후 재승인이 필요합니다.
2. **리프레시 토큰 만료**: 오프라인 액세스를 사용하는 경우 리프레시 토큰도 만료될 수 있습니다.
3. **민감한 스코프 제한**: `gmail.readonly`는 민감한 스코프이므로, 앱이 검증되지 않은 상태에서는 테스트 사용자 제한이 더 엄격합니다.
4. **테스트 사용자 목록 변경**: Google Cloud Console에서 테스트 사용자 목록이 변경되었을 수 있습니다.

**해결 방법:**
1. **Google Cloud Console 확인**:
   - [Google Cloud Console](https://console.cloud.google.com/) 접속
   - **API 및 서비스** > **OAuth 동의 화면** 메뉴로 이동
   - **테스트 사용자** 섹션에서 해당 이메일이 등록되어 있는지 확인
   - 등록되지 않은 경우 다시 추가

2. **재승인 방법** (단계별 가이드):

   **방법 1: 앱에서 다시 로그인 시도 (가장 간단)**
   - 테스트 사용자가 앱에서 로그아웃 후 다시 로그인 시도
   - Google OAuth 동의 화면이 자동으로 나타남
   - "고급" 버튼 클릭
   - "안전하지 않은 페이지로 이동" 또는 "계속" 버튼 클릭
   - 권한 승인

   **방법 2: Google 계정 설정에서 권한 취소 후 재승인**
   - 테스트 사용자가 [Google 계정 설정](https://myaccount.google.com/) 접속
   - **보안** > **Google에 로그인한 기기 및 앱** 메뉴로 이동
   - 또는 **개인정보 및 보안** > **타사 앱 및 서비스** > **타사에서 액세스 권한이 있는 앱** 메뉴로 이동
   - "Workless" 또는 해당 앱 찾기
   - **제거** 또는 **권한 취소** 클릭
   - 앱에서 다시 로그인 시도하면 재승인 화면이 나타남

   **방법 3: 개발자가 Google Cloud Console에서 확인**
   - Google Cloud Console > **API 및 서비스** > **OAuth 동의 화면**
   - **테스트 사용자** 섹션에서 해당 이메일이 여전히 등록되어 있는지 확인
   - 등록되어 있다면 사용자에게 방법 1 또는 2를 안내
   - 등록되지 않았다면 다시 추가

3. **앱 게시** (근본적인 해결):
   - 앱을 게시하고 검토를 받으면 테스트 사용자 제한이 해제됩니다
   - 모든 사용자가 앱을 사용할 수 있게 됩니다

**참고:**
- 테스트 모드에서는 최대 100명의 테스트 사용자만 등록 가능합니다
- 테스트 사용자 승인은 7일마다 만료되므로, 정기적으로 재승인이 필요합니다
- 프로덕션 환경에서는 이 제한이 없습니다
